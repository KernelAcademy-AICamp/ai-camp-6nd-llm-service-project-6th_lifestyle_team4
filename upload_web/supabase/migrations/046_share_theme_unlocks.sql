...
create table if not exists public.share_theme_unlocks (
    ...
    theme_id    text        not null,        -- share_backgrounds.slug (클라 선택 id)
...
-- 소유자만 자기 보유 목록을 읽는다. user_id ↔ auth.uid() 는 users.anonymous_id 로 매핑.
-- 비회원(익명)도 구매 가능하므로(웹 현행 동작) anon 도 자기 행을 읽게 둔다.
...
-- 쓰기는 아래 SECURITY DEFINER RPC 로만.

-- 카드지 구매 — auth.uid() 로 대상 도출 + yarn_balance 에서 p_price 원자적 차감 + 소유 등록.
-- 반환 int:
--    >=0 → 차감 후 yarn_balance (이미 보유면 차감 없이 현재 잔액)
--     -1 → 인자 NULL
--     -2 → 잔액 부족 (미차감)
--     -3 → user 행 없음
...
begin
    ...
    -- atomic 차감 — 잔액 부족이면 0 rows → v_balance NULL
    update public.users
