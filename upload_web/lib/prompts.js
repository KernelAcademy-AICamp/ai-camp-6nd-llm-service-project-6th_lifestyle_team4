...
// ---------------------------------------------------------------------------
//  BOOK_INTRO_PROMPT
//  작품 메타(+본문 일부)로 "이 고전을 왜 읽어야 하는가" 영업 소개 1~2문장 생성.
//  - NEW(새로 들어온 고전) 카드에서 명대사 대신 표시.
...
[규칙]
- 분량: 1~2문장, 한국어로 60~140자.
- 톤: 차분하고 품격 있게, 과장·낚시성 표현 금지("인생작", "강추" 등 금지).
- 내용: 이 작품이 왜 읽을 가치가 있는지 — 문학사적 의의, 다루는 주제, 작가의 위상 중 한두 가지를 자연스럽게 녹인다.
- 결말·핵심 반전 등 스포일러 금지.
- 줄거리 요약이 아니라 "읽고 싶게 만드는" 소개여야 한다.
- 제목·지은이를 문장 안에서 반복 나열하지 말 것(이미 카드에 표시됨). 작품 내용·의의에 집중.
- 확실하지 않은 사실(수상 이력·판매고 등 구체 수치)은 지어내지 말 것.
