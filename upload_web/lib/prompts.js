...
- excerpt_description: 객관적 장면 설명만, 감상 배제. 한국어로.
  · 기본은 1~2줄로 간결하게 (500자 이내).
...
- excerpt_description
  · quote 직전의 흐름을 묘사하는 산문 한 단락. 기본 200~500자.
  · 단, script_excerpt의 앞뒤 텍스트만으로 명문장 이해가 부족할 때(맥락이 적거나 분량이 짧을 때)는 상황 설명을 **길게**(흐름·정조·맥락을 풍부히) 작성해 명문장 이해를 도울 것. 이때는 500자를 넘겨도 됨.
  · 흘러가는 서술체. 라벨/머리표 금지("배경:", "직전 흐름:" 등).
  · 직전 문장을 그대로 인용하지 말 것.
  · 작품 전체 줄거리 소개·감상 표현 금지.
...
- significance
  · 80~200자
  · 이 문장이 작품 전체에서 갖는 의의를 한두 문장으로 응축
  · 가르치려 들지 말 것("독자는 ~를 느낀다" 금지)
  · 카드마다 다른 각도·표현으로 (반복·복제 금지)
  · 모든 카드에 반드시 포함
